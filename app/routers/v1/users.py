...
@router.post(
    "/external/users", status_code=status.HTTP_201_CREATED, response_model=dict
)
async def create_user(user: UserCreate, session: SessionDep) -> UserInDB:
    """Add a user to DB"""
    ...
    return {"email": db_user.email, "status": "created"}


@router.get("/internal/users", response_model=list[UserBase])
async def get_users(
    ...
    return session.exec(
        select(User)
        .where(
            User.created_at >= commons.date_from,
            User.created_at <= commons.date_to
        )
...
@router.patch(
    "/external/users/default_currency", summary="Change current user's default currency"
)
async def change_curr_user_default_currency(
    ...
    currency_db = get_currency_from_db(
        default_currency.default_currency.upper(), session
    )
    if not currency_db:
        raise HTTPException(
            status_code=404,
            detail=f"Currency {default_currency.default_currency} is not supported.",
        )
