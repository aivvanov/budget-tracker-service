...
def get_user_total_balance_in_default_currency(session, user_id, default_currency) -> float | None:
    ...
    total = 0.0
    for acc in accounts:
        curr_amount = convert_currency(session, acc["amount"], acc["currency"], default_currency)
        total += curr_amount
